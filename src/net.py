...
class Net(object):
    ...
    def load_session(self, checkpoint, p, c, memory_fraction):     
        training_schedule = LONG_SCHEDULE
        inputs = {'input_a': p, 'input_b': c}
        ...
        if memory_fraction != -1:
            sess = tf.Session(config = tf.ConfigProto(gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction = memory_fraction), allow_soft_placement = True))
        else:
            sess = tf.Session(config = tf.ConfigProto(allow_soft_placement = True, gpu_options = tf.GPUOptions(allow_growth = True)))
        
        saver.restore(sess, checkpoint)
        return sess, pred_flow
